...
if __name__ == '__main__':
	...
	ns = 15
	# 加载抽样的数据
	nf, ret = import_data_first_iris("datasets/iris202407.txt", ns)
	# 预处理数据
	...
	# print_matrix(sum_w)
    # 最终的wfcm
	okfcm_U, okfcm_V = wfcm(sum_V, 3, 2, sum_w, []) 
	# 加载完整数据
	datafull, cluster_locationfull = import_data_full_iris("datasets/iris202407.txt")
	final_okfcm_U = extension(datafull, 3, 2, okfcm_V)
	# 准确度分析
	print (checker_iris(final_okfcm_U))
