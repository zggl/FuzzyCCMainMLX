...
def initialise_U(data, cluster_number):
	...
	for i in range(0, len(data)):
		...
		for j in range(0, cluster_number):
			...
			# random.randint(a,b)：用于生成一个指定范围内的整数。其中参数a是下限，
            参数b是上限，生成的随机数n：a<=n<=b
...
def checker_dataset(final_location):
	"""
	和真实的聚类结果进行校验比对
	"""
	right = 0.0
...
if __name__ == '__main__':
	...
	# 加载数据
	data, cluster_location = import_data_format(file)
	# print_matrix(data)
	...
	# 还原数据
	kFCM_U = de_randomise_data(final_location, order)
	# print_matrix(final_location)
 
	# 准确度分析
	print (checker_dataset(final_location))
