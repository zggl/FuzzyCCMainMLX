...
def checker_iris_15(final_location):
	"""
	和真实的聚类结果进行校验比对
	"""
	right = 0.0
...
if __name__ == '__main__':
	...
	ns = 5
	# 加载抽样的数据
	nf, ret = import_data_first_iris("datasets/iris202407_15.txt", ns)
	# 预处理数据
	...
	start = time.time()
	# 调用模糊C均值函数
	spFCM_U, V = wfcm(data, 3, 2, w, [])
	print("-----------------------------------------")
	# 剩余数据集的迭代
	spfcm_V = ilteral_all(ret, ns, spFCM_U, V, w, 3)
	# 加载完整数据
	datafull, cluster_locationfull = import_data_full_iris("datasets/iris202407_15.txt")
	final_spFCM_U = extension(datafull, 3, 2, spfcm_V)
	# 准确度分析
	print (checker_iris_15(final_spFCM_U))
